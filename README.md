...
> **포트 안내**: 본 프로젝트는 MySQL을 **3307 포트**로 설정했습니다.
> MySQL 서버를 3307 포트로 실행하거나, 위 `spring.datasource.url`의 포트 번호를 본인 환경에 맞게 변경하세요.
...
### 5. 동일 월 다수 취소 케이스 (`creator1_4월_동일월_다수취소`)

**내용**: `creator-1`의 2025년 4월 정산 조회

- 4월 판매: 3건 (100,000 + 100,000 + 50,000 = 250,000원)
- 4월 취소: 2건 (100,000 + 50,000 = 150,000원, 같은 달 내 발생)
- 예상 결과: 순 판매 100,000원 / 수수료 15,000원(15%) / 정산 예정 85,000원

**추가 이유**: 한 달 안에 여러 건의 취소가 발생할 때 환불 금액이 올바르게 합산되는지 검증하기 위해 추가했습니다. 또한 2025-04부터 수수료율이 20%→15%로 변경되므로, 수수료 정책 변경이 실제로 적용되는지도 함께 확인합니다.

---

### 6. 잘못된 연월 형식 예외 케이스 (`잘못된_연월_형식_예외`)

**내용**: month 파라미터에 `2025/03`처럼 잘못된 형식 전달

- 예상 결과: `ResponseStatusException(400 Bad Request)` 발생

**추가 이유**: API 입력값 검증이 올바르게 동작하는지 확인하기 위해 추가했습니다. `yyyy-MM` 형식이 아닌 값이 들어올 때 서버 오류(500)가 아닌 명확한 400 응답을 반환하는지 검증합니다.
